...
def export_from_progress(source_file: Path, out_dir: Path, store: ProgressStore, force_export: bool = False, block_on_pending_error: bool = False, metadata: dict | None = None):
    started = now_ts(); original = parse_file(source_file); store.apply_existing(original)
    ...
    for l in original:
        if l.separator == "|":
            ...
            l.english_part = pt; l.original_line = f"{l.chinese_part}|{pt}"
        reconstructed.append(l)
    validation = validate_lines(parse_file(source_file), reconstructed)
    ...
    if validation.validation_passed or force_export:
        with out_file.open("w", encoding="utf-8", newline="") as f:
            for l in reconstructed:
                f.write((f"{l.chinese_part}|{l.english_part}" if l.separator == "|" else l.original_line) + l.original_line_ending)
    report = _build_report(source_file, out_file, store, reconstructed, validation.to_dict(), started, metadata, force_export)
